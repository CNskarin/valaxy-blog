...
const { site: track } = useAddonVercount()
...
<template>
  ...
  >
    ...
    <ClientOnly>
      <div class="site-vercount-stats" flex="~ col" items="center" m="y-1" gap="1">
        <span class="inline-flex items-center gap-2">
          <span class="inline-flex items-center gap-1" title="全站访问量">
            <i class="i-ri-eye-line" />
            <span>访问 {{ track?.pv }}</span>
          </span>
          <span class="op-40">·</span>
          <span class="inline-flex items-center gap-1" title="全站访客数">
            <i class="i-ri-user-line" />
            <span>访客 {{ track?.uv }}</span>
          </span>
        </span>
      </div>
    </ClientOnly>
